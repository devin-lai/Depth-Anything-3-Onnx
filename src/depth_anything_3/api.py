...
try:
    ...
    # Export is optional for CoreML/TorchScript export paths that don't need exporters
    export = None
...
try:
    ...
    def align_poses_umeyama(*args, **kwargs):
        raise ImportError("align_poses_umeyama requires optional dependency 'evo'")
...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        """
        # Determine optimal autocast dtype based on device
        device_type = image.device.type
        if device_type == "cuda":
            autocast_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
        else:
            # For CPU and other devices, use bfloat16 if available, otherwise float32
            autocast_dtype = torch.bfloat16 if hasattr(torch, 'bfloat16') else torch.float32
        ...
        with torch.no_grad():
            ...
            if device_type == "cuda":
                ...
                # For CPU, run without autocast for better compatibility
                return self.model(image, extrinsics, intrinsics, export_feat_layers, infer_gs)

    def inference(
        self,
        image: np.ndarray | Image.Image | str | list[np.ndarray | Image.Image | str],
        ...
        # Preprocess images
        imgs_cpu, extrinsics, intrinsics, orig_hw = self._preprocess_inputs(
        ...
        prediction = self._add_processed_images(prediction, imgs_cpu, orig_hw)

        # Export if requested
        if export_dir is not None and export_format is not None:
    ...
    def _preprocess_inputs(
        ...
        process_res: int = 504,
        process_res_method: str = "upper_bound_resize",
    ) -> tuple[torch.Tensor, torch.Tensor | None, torch.Tensor | None, list[tuple[int, int]]]:
        """Preprocess input images using input processor."""
        start_time = time.time()
        orig_hw = [self._get_image_hw(img) for img in image]
        ...
        return imgs_cpu, extrinsics, intrinsics, orig_hw
    ...
    def _run_model_forward(
        ...
        """Run model forward pass."""
        device = imgs.device
        need_sync = device.type == "cuda" and torch.cuda.is_available()
        if need_sync:
    ...
    def _add_processed_images(
        self, prediction: Prediction, imgs_cpu: torch.Tensor, orig_hw: list[tuple[int, int]]
    ) -> Prediction:
        """Add processed images and original sizes to prediction for visualization."""
        ...
        prediction.orig_hw = np.asarray(orig_hw, dtype=np.int32)
        return prediction

    def _get_image_hw(self, img: np.ndarray | Image.Image | str) -> tuple[int, int]:
        """Return (H, W) of the original input image without modifying it."""
        if isinstance(img, str):
            with Image.open(img) as im:
                w, h = im.size
            return h, w
        if isinstance(img, np.ndarray):
            # Assume H x W x C or H x W
            return int(img.shape[0]), int(img.shape[1])
        if isinstance(img, Image.Image):
            w, h = img.size
            return h, w
        raise ValueError(f"Unsupported image type for size extraction: {type(img)}")
    ...
    def _export_results(
        ...
        if export is None:
            raise ImportError(
                "depth_anything_3.utils.export dependencies are missing; "
                "install optional export deps to enable exporting."
            )
        start_time = time.time()
