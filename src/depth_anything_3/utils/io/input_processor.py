...
class InputProcessor:
    ...
    def _process_one(
        ...
        *,
        ...
        # Boundary resize (or fixed square resize)
        ...
        if process_res_method.endswith("resize") or process_res_method == "fixed_square":
    ...
    def _resize_image(self, img: Image.Image, target_size: int, method: str) -> Image.Image:
        ...
        elif method == "fixed_square":
            return self._resize_to_square(img, target_size)
        else:
    ...
    def _resize_to_square(self, img: Image.Image, target_size: int) -> Image.Image:
        """
        Resize to an exact square of (target_size, target_size).
        Keeps per-axis scaling equal so the final shape is fixed.
        """
        upscale = target_size > max(img.size)
        interpolation = cv2.INTER_CUBIC if upscale else cv2.INTER_AREA
        arr = cv2.resize(np.asarray(img), (target_size, target_size), interpolation=interpolation)
        return Image.fromarray(arr)
